...
export default function ApprovalModal({ plan, sessionId, onDecision }) {
  ...
  const handleSubmitModification = async () => {
    ...
    // Tell App.jsx: not approved, but has modifications
    // Phase stays "running", WebSocket will send new awaiting_approval
    onDecision(false, true);
  };

  const handleCleanCancel = async () => {
    setLoading(true);
    await fetch(`${API_BASE}/api/research/approve`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    // Tell App.jsx: cancelled with no modifications
    // WebSocket "cancelled" event will reset phase to idle
      ...
      >
          ...
          {showModify && (
            <div className="mt-4">
              ...
              <textarea
                value={modifications}
                onChange={(e) => setModifications(e.target.value)}
                placeholder="e.g. Focus more on developing countries, include 2024 data..."
                className="w-full p-3 rounded-xl text-sm resize-none"
                rows={3}
                style={{
                  border: "2px solid #E2E8F0",
              ...
              />
              {/* Submit modification button — only shown when text is typed */}
              {modifications.trim() && (
                <button
                  onClick={handleSubmitModification}
                  disabled={loading}
                  className="mt-2 w-full py-2 rounded-xl text-sm font-bold text-white transition-all disabled:opacity-50"
                  style={{ background: "linear-gradient(135deg, #F59E0B, #EF4444)" }}
                >
                  {loading ? "Sending..." : "Submit Modification & Replan"}
                </button>
              )}
            </div>
          )}
        ...
        >
          <button
            ...
            className="text-sm font-medium px-4 py-2 rounded-xl transition-all"
            style={{
              color: showModify ? "#0EA5E9" : "#64748B",
              background: showModify ? "#E0F2FE" : "#F8FAFC",
              border: `1px solid ${showModify ? "#BAE6FD" : "#E2E8F0"}`,
            }}
          >
            {showModify ? "Hide Modifications" : "Request Modifications"}
          </button>
